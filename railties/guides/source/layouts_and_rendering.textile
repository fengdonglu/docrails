...
You've heard that Rails promotes "convention over configuration". Default rendering is an excellent example of this. By default, controllers in Rails automatically render views with names that correspond to valid routes. 例如， if you have this code in your +BooksController+ class:
...
如果我们希望 to display the properties of all the books in our view, we can do so with an ERB template like this:
...
NOTE: 实际上渲染由 +ActionView::TemplateHandlers+ 的子类完成。 本篇不打算深入处理过程，但是 it's important to know that the file extension on your view controls the choice of template handler. 从 Rails 2 开始，标准扩展名 are +.erb+ for ERB (HTML with embedded Ruby), and +.builder+ for Builder (XML生成器).
...
The +javascript_include_tag+ helper returns an HTML +script+ tag for each source provided. Rails looks in +public/javascripts+ for these files by default, but you can specify a full path relative to the document root, or a URL, if you prefer. 例如， to include +public/javascripts/main.js+:

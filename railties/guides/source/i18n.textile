...
The Ruby I18n (shorthand for _internationalization_) gem which is shipped with Ruby on Rails (从2.2版开始) provides an easy-to-use and extensible framework for *translating your application to a single custom language* other than English or for *提供多语言支持* in your application.
...
好了，下一章，让我们从无到有，国际化一个简单的 Rails 应用程序 from the ground up ！
...
这个目录中默认的 +en.yml+ 区域包含了 a sample pair of translation strings:
...
I18n库会使用 *英语* 作为 *默认的区域*, i.e. if you don't set a different locale, +:en+ will be used for looking up translations.
...
另一种从用户信息中选择区域的方法，是使用映射客户端 IP 至区域的数据库，比如 "GeoIP Lite Country":http://www.maxmind.com/app/geolitecountry. 代码技巧和之前很类似 -- you would need to query the database for the user's IP, and look up your preferred locale for the country/region/city returned.

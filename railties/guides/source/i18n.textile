h2. Rails 国际化 (I18n) API

The Ruby I18n ( _internationalization_ 的缩写) gem 由 Ruby on Rails （从2.2版开始）默认提供，提供了容易使用的和可扩展的框架，可以在应用程序中，除了英语之外， *把你的应用程序翻译成一个单一的用户语言* ，或者 *提供多语言支持* 。

国际化处理通常意味着从应用程序中提取字符串和其它区域相关的信息（比如日期或者货币格式）。本地化处理意味着提供语言文件和这些信息的本地化格式。 [1]

所以，在国际化你的Rails应用程序的过程中，你必须：

* 确保支持国际化
* 告诉 Rails 如何查找区域词典文件
* 告诉 Rails 如何设置，维持，切换区域

在本地化的过程中，你也许想做以下三件事：

* 替换或者补充 Rails 的默认区域 -- 例如日期和时间格式，月份名称， Active Record 模型名称等等。
* 把应用程序中的字符串提取到字典 -- 例如闪动的消息、视图中的静态文本等等。
* Store the resulting dictionaries somewhere

本篇将带你体验国际化的接口，还包含了一个如何从头开始国际化一个应用程序的教程。

endprologue.

NOTE: Ruby I18n 框架提供了国际化/本地化所需的一切。你可以，然而， use any of various plugins and extensions available, which add additional functionality or features. See the Rails "I18n Wiki":http://rails-i18n.org/wiki for more information.

h3. How I18n in Ruby on Rails Works

国际化是一个复杂的问题。自然语言提供如此多的表达方式（例如复数规则），所以不能用工具解决所有的问题。因此Rails 的国际化接口着重于：

* 内建支持英语或类似的语言
* 为其它语言的定制和扩展提供必要的帮助

作为解决方案的一部分， *Rails框架中的所有静态字符串* -- 例如 Active Record 的校验消息，日期和时间格式等 -- *都已经被国际化了* ， so _localization_ of a Rails application means "over-riding" these defaults.

h4. 库总体架构

总的来说，Ruby I18n gem 分为两部分：

* The public API of the i18n framework -- a Ruby module with public methods that define how the library works
* 一个默认的后端 (which is intentionally named _Simple_ backend) 来实现这些方法

作为用户，你永远只需要访问I18n模块的公共方法，但了解后端的能力也是很有用的。

NOTE: 可以把简单的后端替换为一个更强大的后端（虽然并不可取），它可在关系型数据库、GetText 字典、或者类似的东西中存储翻译文件。 查看下面的 "使用不同的后端":#using-different-backends 小节。
...
好了，下一章，让我们从无到有，国际化一个简单的 Rails 应用程序！

h3. 应用程序国际化准备

只需一些简单的步骤就可以为应用程序建立并运行I18n支持。

h4. 配置 I18n 模块

遵守 _约定重于配置_ 的哲学， Rails 会合理设置你的应用程序的默认值。如果你需要不同的设置，你可以轻易地覆盖它们。

Rails 自动地从 +config/locales+ 目录中把所有的 +.rb+ 和 +.yml+ 文件加入你的 *translations load path* 。

这个目录中默认的 +en.yml+ 区域文件包含了一对简单的字符串：
...
这意味着， that in the +:en+ locale, the key _hello_ will map to the _Hello world_ 字符串。每一条 Rails 中的字符串都是用这种方式进行国际化， see for instance Active Record validation messages in the "+activerecord/lib/active_record/locale/en.yml+":https://github.com/rails/rails/blob/master/activerecord/lib/active_record/locale/en.yml file or time and date formats in the "+activesupport/lib/active_support/locale/en.yml+":https://github.com/rails/rails/blob/master/activesupport/lib/active_support/locale/en.yml file. 你可以使用 YAML 或者标准的 Ruby Hashes 来存储 translations in the default (Simple) backend.

I18n库会使用 *英语* 作为 *默认的区域* ，也就是说，如果没有设置区域， +:en+ 会用于查找语言文件。
...
*语言文件载入路径* (+I18n.load_path+) is just a Ruby Array of paths to your translation files that will be loaded automatically and available in your application. You can pick whatever directory and translation file naming scheme makes sense for you.
...
如果你想把Rails应用程序翻译到一个 *单一的语言但不是英语* （默认的区域），你可以 set I18n.default_locale to your locale in +application.rb+ or an initializer as shown above, and it will persist through the requests.
...
One option you have is to set the locale from the domain name where your application runs. 例如，我们希望 +www.example.com+ 载入英语（或者默认）区域， 而 +www.example.es+ 载入西班牙区域。 Thus the _top-level domain name_ is used for locale setting. 这有几个优点：

* The locale is an _obvious_ part of the URL.
* People intuitively grasp in which language the content will be displayed.
* It is very trivial to implement in Rails.
* 搜索引擎好像比较喜欢 that content in different languages lives at different, inter-linked domains.

你可以在 +ApplicationController+ 中这样实现它：
...
你也可以用很简单的方式，设置区域 from the _subdomain_ 。
...
如果你的应用程序包含了区域切换菜单，你可以象这样做：

<ruby>
link_to("Deutsch", "#{APP_CONFIG[:deutsch_website_url]}#{request.env['REQUEST_URI']}")
</ruby>

假定你已经设定了 +APP_CONFIG[:deutsch_website_url]+ 为象 +http://www.application.de+ 这样的值。

这种方案的优势上面已经说过了，但是，你无法或者不希望在不同的域名上提供提供不同的区域（“语言版本”）。 The most obvious solution would be to include locale code in the URL params (or request path).
...
你会对此感到满意。 It does impact the readability of URLs, though, when the locale "hangs" at the end of every URL in your application. Moreover, from the architectural standpoint, locale is usually hierarchically above the other parts of the application domain: and URLs should reflect this.
...
当然，你需要 you need to take special care of the root URL (usually "homepage" or "dashboard") of your application. An URL like +http://localhost:3001/nl+ will not work automatically, because the +root :to => "books#index"+ declaration in your +routes.rb+ doesn't take locale into account. (And rightly so: there's only one "root" URL.)
...
NOTE: 花一点时间看一下这两个插件，它们会简化这种方式下的路由工作： Sven Fuchs's "routing_filter":https://github.com/svenfuchs/routing-filter/tree/master 和 Raul Murciano 的 "translate_routes":https://github.com/raul/translate_routes/tree/master.
...
h5. 使用 GeoIP (或类似的)数据库

另一种从用户信息中选择区域的方法，是使用映射客户端 IP 至区域的数据库，比如 "GeoIP Lite Country":http://www.maxmind.com/app/geolitecountry 。代码技巧和之前很类似 -- 你只需要在数据库中查询用户的 IP 地址，and look up your preferred locale for the country/region/city returned.
...
显然 there are *two strings that are localized to English*. In order to internationalize this code, *replace these strings* with calls to Rails' +#t+ helper with a key that makes sense for the translation:
...
渲染这个视图时，它会显示一条出错消息，告诉你找不到 the translations for the keys +:hello_world+ and +:hello_flash+ 。
...
NOTE: 增加了新的区域文件后，需要重启服务器

你可以使用 YAML (+.yml+) 或者标准的 Ruby (+.rb+) 文件存储你的语言文件 in SimpleStore. YAML is the preferred option among Rails developers. However, it has one big disadvantage. YAML is very sensitive to whitespace and special characters, so the application may not load your dictionary properly. Ruby files will crash your application on first request, so you may easily find what's wrong. (If you encounter any "weird issues" with YAML dictionaries, try putting the relevant portion of your dictionary into a Ruby file.)
...
You should have good understanding of using the i18n library now, 知道了国际化一个基本的应用程序所需的所有方面。 下面的这章中，我们将在更深的层次 cover it's features。
...
如果设置了 +:default+ 选项， its value will be returned if the translation is missing:
...
Keys with a '_html' suffix and keys named 'html' are marked as HTML safe. 在视图中使用它们不需要转义。
...
Active Support 自带的Simple后台允许你使用普通的Ruby和YAML格式存储你的语言文件。 [2]

例如，一个Ruby Hash 提供的语言文件，看起来象这样：
...
一般来说，我们推荐使用 YAML 作为存储语言文件的格式。 There are cases, though, where you want to store Ruby lambdas as part of your locale data, e.g. for special date formats.
...
例如， you might have an Admin model inheriting from User:
...
h5. 错误信息Interpolation

The translated model name, translated attribute name, and value are always available for interpolation.

So, 例如， instead of the default error message +"can not be blank"+ you could use the attribute name like this : +"Please fill in your %{attribute}"+.
...
Rails 使用固定的字符串和其它 localizations, such as format strings and other format information in a couple of helpers. Here's a brief overview.
...
这当然不是说，你必须忍受这些限制。 Ruby I18n gem 可以容易地把简单的后端替换成你需要的。 例如，你可以把它替换成 Globalize's Static 的后端：

<ruby>
I18n.backend = Globalize::Backend::Static.new
</ruby>

你也可以使用 Chain 来串联多个后端。如果你想在默认的后端中存储默认的语言文件，又在数据库或者其它后端中存储了定制的语言文件时，会很有用。例如，你可以使用 Active Record 后端，然后是默认的 Simple 后端：
...
Ruby on Rails的国际化支持从版本 2.2 开始引进，仍在发展。The project follows the good Ruby on Rails development tradition of evolving solutions in plugins and real applications first, and only then cherry-picking the best-of-breed of most widely useful features for inclusion in the core.

因此我们鼓励每个人来体验插件或其它库中的新想法和特性，为社区打磨它们。 (别忘了在邮件列表中通报：http://groups.google.com/group/rails-i18n)
...
如果你觉得本篇有用， please consider recommending its authors on "workingwithrails":http://www.workingwithrails.com.

...
本篇说明了 Rails 3.1 内部初始化过程。这是进阶读物，推荐高级开发者。
...
在 Rails 3 中， +script/server+ 已经变成了 +rails server+ 。 This was done to centralize all rails related commands to one common file.
...
这个程序尝试装载 +rails/cli+ ，如果找不到，it then add the +railties/lib+ path to the load path (+$:+) 后重试。

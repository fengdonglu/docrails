...
Rails 内置的 Ajax/JavaScript 功能（和更多）；它让你轻易地创建丰富的动态 AJAX 应用程序！将包含以下主题：
...
To get access to these helpers, all you have to do is to include the prototype framework in your pages - 一般是在你的主布局页面， application.html.erb - like so:
...
如果服务器返回 200, the output of the above example is equivalent to our first, simple one. However, in case of error, the element with the DOM id +error+ is updated rather than the +cart+ element.
...
JavaScript 测试让我们想起了Mark Twain对'classic'的定义： "A classic is something that everybody wants to have read and nobody wants to read." It's similar with JavaScript testing: everyone would like to have it, yet it's not done by too much developers as it is tedious, complicated, there is a proliferation of tools and no consensus/accepted best practices, but we will nevertheless take a stab at it:

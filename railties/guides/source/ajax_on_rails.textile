...
Rails 内置了 Ajax/JavaScript 功能（和更多）；它让你轻易地创建丰富的动态 AJAX 应用程序！本篇将包含以下话题：

* AJAX的快速介绍和相关技术
* Handling JavaScript the Rails way: Rails 辅助方法、 Prototype 和 script.aculo.us
* 测试 JavaScript 功能

endprologue.

h3. Hello AJAX - 一份快速介绍

你需要了解 DOM、HTTP请求和其它这里讨论主题的基本知识，以真正了解Ajax on Rails。

h4. 异步 JavaScript + XML

基本术语，创建web应用程序的新风格

h4. DOM

DOM的基础, how is it built, properties, features, why is it central to AJAX

h4. 标准 HTML 通讯 vs AJAX
...
Rails选择的的JavaScript框架是 "Prototype":http://www.prototypejs.org. Prototype是一个通用的JavaScript框架， 目的是通过操作DOM来简化web应用程序开发 AJAX 和其它 JavaScript 功能 ranging from utility functions to object oriented constructs. 它并未为任何特定语言编写，所以 Rails 提供了一套辅助方法， 将Prototype 无缝集成至你的 Rails 视图。

要访问这些辅助方法，你要做的所有事情，就是在你的页面中包含 prototype 框架 - 一般是在你的主布局页面， application.html.erb - 象这样：
...
h4. AJAX Rails 辅助方法的精髓： link_to_remote
...
如果服务器返回 200, 上面示例的输出 is equivalent to our first, simple one. However, in case of error, the element with the DOM id +error+ is updated rather than the +cart+ element.
...
使用 +:bottom+ 的典型例子就是在已存在的列表中插入一个新的 &lt;li&gt; 元素：
...
我们已经完成了 +link_to_remote+ 。 我知道 this is quite a lot to digest for one helper function, but remember, these options are common for all the rest of the Rails view helpers, so we will take a look at the differences / additional parameters in the next sections.

h4. AJAX 表单

有三种不同的办法使用 Rails Prototype 辅助方法将AJAX表单加入到视图中。它们有一些细微的区别，但是都是为了相同的目标：代替使用标准 HTTP request/response cycle的表单提交，它是异步提交，因此不刷屏。这些方法是：

* +remote_form_for+ (别名： +form_remote_for+) is tied to Rails most tightly of the three since it takes a resource, model or array of resources (in case of a nested resource) as a parameter.
...
(Note: 如果你想测试上述方法，创建 +link_to_remote+ 带着一个参数 - +:url+ ，指向 +javascript_test+ action)

What happens here is that by specifying the Content-Type header variable, 我们要求浏览器 to evaluate the text we are sending over (rather than displaying it as plain text, which is the default behavior).

h3. 测试 JavaScript

JavaScript 测试让我们想起了Mark Twain对于'classic'的定义： "A classic is something that everybody wants to have read and nobody wants to read." 。在 JavaScript 测试中也是这样： everyone would like to have it, yet it's not done by too much developers as it is tedious, complicated, there is a proliferation of tools and no consensus/accepted best practices, but we will nevertheless take a stab at it:

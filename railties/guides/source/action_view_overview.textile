...
* Action View是什么，怎么在Rails中使用它
* 怎么在Rails之外使用Action View
* 怎么更好地使用模板, partials, and layouts
...
Action View和Action Controller是Action Pack中两个主要的组件。 在 Rails 中, web请求由 Action Pack 获取, 它把工作分为控制器部分 (performing the logic)和视图部分（渲染模板） 。 典型地， Action Controller将在必要时参与数据库和performing CRUD actions之间的通讯。Action View然后负责响应。

Action View模板使用嵌入式Ruby编写，在HTML内混入标签。 To avoid cluttering the templates with boilerplate code, 一些辅助方法类提供了表单、日期和字符串的常见行为。在你的应用程序程序进化过程中加入新的辅助方法也很容易。

Note: 一些Action View的特性和Active Record有关，但是这并不是说Action View 基于 Active Record。 Action View 是一个独立的软件包that can be used with any sort of backend.
...
Action View模板可以使用几种办法编写。 如果模板文件是 <tt>.erb</tt> 后缀， then it uses a mixture of ERB (included in Ruby) and HTML. 如果模板文件是 <tt>.builder</tt> 后缀， then a fresh instance of <tt>Builder::XmlMarkup</tt> library is used.
...
By default, Rails will compile each template to a method in order to render it. 当你修改一个模板， Rails 会检查文件的修改时间，重新编译 in development mode.

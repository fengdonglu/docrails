...
本篇不会教你编写 Rails 应用程序；它假定你已经基本熟悉了 Rails 。

endprologue.

h3. 为什么要为你的 Rails 应用程序编写测试？
...
在每个Rails应用程序中，你都建立了三种环境：生产、开发、测试。
...
例如，假定你需要测试你新的 +delete_this_user_and_every_everything_associated_with_it+ 功能。 Wouldn't you want to run this in an environment where it makes no difference if you destroy data or not?
...
当你使用 +rails new+ _application_name_ 创建一个 Rails 项目时， Rails 为你创建了一个 +test+ 文件夹。这个文件夹中的内容：
...
YAML格式的夹具是非常人性化的方式 to describe your sample data. These types of fixtures have the *.yml* file extension (as in +users.yml+).
...
ERB 允许你在模板中内置 ruby 代码。 YAML 夹具格式 is pre-processed with ERB when you load fixtures. This allows you to use Ruby to help you generate some sample data.
...
内置的 +test/unit+ based testing is not the only way to test Rails applications. Rails developers have come up with a wide variety of other approaches and aids for testing, including:

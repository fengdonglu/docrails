...
象这样不带参数， it creates a +&lt;form&gt;+ tag which, when submitted, will POST to the current page. For instance, assuming the current page is +/home/index+, the generated HTML will look like this (some line breaks added for readability):
...
Many apps grow beyond simple forms editing a single object. 例如当创建一个人，你会考虑允许用户 (使用同样的表单) 创建多个地址记录（家庭、工作等等）。 When later editing that person the user should be able to add, remove or amend addresses as necessary. While this guide has shown you all the pieces necessary to handle this, Rails does not yet have a standard end-to-end way of accomplishing this, but many have come up with viable approaches. These include:

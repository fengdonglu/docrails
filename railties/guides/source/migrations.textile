...
在我们深入迁移的细节之前，这里有一些排序的例子，你可以做做看：
...
这次迁移加入了一张 +products+ 表，带有 +name+ 的字符串字段，称为 +description+ 的文本字段。 主键 column called +id+ will also be added, however since this is the default we do not need to ask for this. The timestamp columns +created_at+ and +updated_at+ which Active Record populates automatically will also be added. Reversing this migration is as simple as dropping the table.
...
一个迁移是 <tt>ActiveRecord::Migration</tt> 的子类，实现了两个方法： +up+ (perform the required transformations) and +down+ (revert them).
...
如果你需要实现任务specific to your database (for example create a "foreign key":#active-record-and-referential-integrity constraint) then the +execute+ function allows you to execute arbitrary SQL. A migration is just a regular Ruby class so you're not limited to these functions. For example after adding a column you could write code to set the value of that column for existing records (if necessary using your models).
...
偶尔你会写错迁移。如果你已经运行了迁移 then you cannot just edit the migration and run the migration again: Rails thinks it has already run the migration and so will do nothing when you run +rake db:migrate+. You must rollback the migration (for example with +rake db:rollback+), edit your migration and then run +rake db:migrate+ to run the corrected version.
...
模型和脚手架generators will create migrations appropriate for adding a new model. This migration will already contain instructions for creating the relevant table. If you tell Rails what columns you want then statements for adding those will also be created. For example, running
...
如果你基于其它原因创建迁移（比如在已有表中增加列），你可以使用 the migration generator:
...
You are not limited to one magically generated column, 比如
...
默认情况下，迁移只是告诉你它做了什么，花了多长时间。 A migration creating a table and adding an index might produce output like this
...
有两种方法导出模式。 This is set in +config/application.rb+ by the +config.active_record.schema_format+ setting, which may be either +:sql+ or +:ruby+.
